...
function prefsBrief(p) {
  ...
  if (p.fodmap) parts.push(
    'ВАЖНО — владелец на диете Low-FODMAP (назначено врачом). Строго избегай продуктов с ВЫСОКИМ FODMAP: ' +
      ...
      meals: {
        ...
        items: {
          ...
          properties: {
            ...
            fodmap: { type: 'string', enum: ['low', 'mod', 'high'], description: 'Уровень FODMAP блюда (только если владелец на Low-FODMAP): low — безопасно, mod — умеренно, high — триггер' },
